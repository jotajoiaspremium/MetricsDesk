...
const isInactive=es=>!["ACTIVE"].includes(es);
...
const Ic={
  ...
  lead:()=><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.61 3.35 2 2 0 0 1 3.6 1h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.6a16 16 0 0 0 6 6l.92-.92a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 21.72 16"/></svg>,
...
.pill.active-on{border-color:${T.green}60;background:${T.green}12;color:${T.green};}
...
function Stat({label,value,dlt,icon:I,hi,neutral,invert}){
  ...
  return(
    <div className="ch stat-card" style={{background:T.s1,border:`1px solid ${T.border}`,borderRadius:10,padding:"16px 18px",transition:"all 0.2s",cursor:"default"}}>
...
function StatusFilter({value,onChange}){
  return(
    <div style={{display:"flex",gap:4}}>
      <button className={`pill${value==="all"?" on":""}`} onClick={()=>onChange("all")}>Todos</button>
      <button className={`pill${value==="active"?" active-on":""}`} onClick={()=>onChange("active")}>Ativos</button>
      <button className={`pill${value==="inactive"?" inactive-on":""}`} onClick={()=>onChange("inactive")}>Inativos</button>
    </div>
  );
}
...
function AccountSwitcher({accounts,activeId,onSwitch,onAdd,onRemove}){
  ...
  return(
    <div ref={ref} style={{position:"relative"}}>
      <button onClick={()=>setOpen(p=>!p)} style={{display:"flex",alignItems:"center",gap:6,background:T.s1,border:`1px solid ${T.border}`,color:T.text,padding:"5px 10px",borderRadius:7,cursor:"pointer",fontSize:12,fontFamily:"'JetBrains Mono',monospace"}}>
...
export default function App(){
  ...
  const dateLabel=[{l:"Hoje",v:"today"},{l:"Ontem",v:"yesterday"},{l:"7 dias",v:"last_7d"},{l:"14 dias",v:"last_14d"},{l:"30 dias",v:"last_30d"},{l:"Este mês",v:"this_month"},{l:"Mês passado",v:"last_month"}].find(r=>r.v===dateRange)?.l||"";
  const accountName=isDemo?"Demo — MetricsDesk":activeAcc?.name||"";
  const tabLabel=["Visão Geral","Campanhas","Conjuntos","Criativos"][["overview","campaigns","adsets","creatives"].indexOf(tab)]||tab;

  const loadDemo=()=>{setIsDemo(true);setOv(D_OV);setOvPrev(D_OV_PREV);setDaily(getDailyDemo("last_30d"));setCamps(D_CAMPS);setAdsets(D_ADSETS);setCreatives(D_CREATIVES);setScreen("dashboard");};
  ...
  return(
    <div style={{background:T.bg,minHeight:"100vh",fontFamily:"'Syne',sans-serif",color:T.text}}>
      ...
      <header style={{position:"sticky",top:0,zIndex:100,height:52,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 24px",borderBottom:`1px solid ${T.border}`,background:`${T.bg}f0`,backdropFilter:"blur(12px)"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:7}} className="no-print">
      ...
      <main style={{padding:"22px 24px 48px",animation:"fadeUp 0.3s ease"}}>
        {loading?<Spinner msg={loadMsg}/>:error?(
          ...
          <div style={{background:T.s1,border:`1px solid ${T.border}`,borderRadius:12,overflow:"hidden"}}>
            ...
            {!hasInsights(sortedAdsets)&&sortedAdsets.length>0&&<div style={{padding:"16px 18px"}}><NoData period={dateLabel}/></div>}
            <div style={{overflowX:"auto"}}>
              <table style={{width:"100%",borderCollapse:"collapse"}}>
